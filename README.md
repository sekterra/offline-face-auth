...
```java
// MobileFaceNet 기본값이 builder()에 내장되어 있어 최소 설정으로 동작
FaceAuthConfig config = FaceAuthConfig.builder()
    ...
    .embeddingDim(192)
    .modelOutputIsNormalized(true)      // MobileFaceNet은 L2 정규화 출력
...
### TFLite 모델 배치 — MobileFaceNet

```
faceauth-sdk/src/main/assets/mobilefacenet.tflite
```

#### 다운로드 경로 (권장)

```
https://github.com/JuheonYi/TFLiteFaceExample
→ app/src/main/assets/mobile_face_net.tflite
```

파일을 받은 후 이름을 `mobilefacenet.tflite`로 변경해서 `assets/`에 배치.

#### MobileFaceNet 스펙 (SDK에 이미 반영됨)
