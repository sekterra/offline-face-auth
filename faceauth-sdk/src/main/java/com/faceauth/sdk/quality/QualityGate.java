...
public final class QualityGate {
    ...
    private static float computeBlurScore(Bitmap bmp) {
        // 간소화 구현: 그레이스케일 변환 후 Laplacian 근사
        int w = Math.min(bmp.getWidth(),  320);
        int h = Math.min(bmp.getHeight(), 320);
    ...
    private static float computeBrightness(Bitmap bmp) {
        int w = Math.min(bmp.getWidth(),  160);
        int h = Math.min(bmp.getHeight(), 160);
        int[] pixels = new int[w * h];
        bmp.getPixels(pixels, 0, bmp.getWidth(), 0, 0, w, h);
        double sum = 0;
        for (int p : pixels) sum += gray(p);
